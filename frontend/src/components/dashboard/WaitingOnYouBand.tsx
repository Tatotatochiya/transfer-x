...
export default function WaitingOnYouBand({
  ...
  return (
    <Card tier={1} noPadding className="mb-[18px]">
      ...
      <div>
        {shown.map((item) => (
          <div key={item.key} className="flex flex-wrap items-center gap-4 border-b border-rule px-5 py-3.5 last:border-b-0">
            ...
            <button
              ...
              className="whitespace-nowrap rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent-hover transition-colors"
            >
              {item.actionLabel}
            </button>
          </div>
