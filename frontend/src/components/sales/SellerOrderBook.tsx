...
const STATUS_VARIANT: Record<string, string> = {
  ...
  SENT:      "bg-success/20 text-success-text",
  COUNTERED: "bg-warning-fill/20 text-warning-text",
  ACCEPTED:  "bg-accent/20 text-accent",
  REJECTED:  "bg-danger/20 text-danger-text",
  WITHDRAWN: "bg-text-muted/15 text-text-muted",
  EXPIRED:   "bg-text-muted/15 text-text-muted",
  OUTBID:    "bg-text-muted/10 text-text-muted",
};
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-border bg-surface-inset">
        ...
        {ob.summary ? (
          <p className="text-sm text-text">{ob.summary}</p>
        ) : (
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeEntries.map((entry) => (
          <div
            ...
            className={`relative group px-4 py-3 border-b border-rule-faint cursor-pointer transition-colors overflow-hidden ${
              selectedId === entry.id
                ? "bg-success/10 ring-inset ring-1 ring-success/30"
            ...
            <div className="relative flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                {entry.club?.crest_url ? (
                  ...
                  <div className="h-6 w-6 rounded bg-surface-inset flex items-center justify-center shrink-0 text-xs font-bold text-text-muted">
                ...
                {entry.is_countered && (
                  <span className="ml-1 text-xs text-warning-text-alt shrink-0">↔</span>
                )}
              </div>
            ...
            <div className="relative mt-1.5 flex items-center justify-between pl-9">
              ...
              {isOpen && saleType === "AUCTION" && (
                <button
                  ...
                  className="text-xs px-2 py-0.5 rounded bg-success/15 text-success-text ring-1 ring-success/30 hover:bg-success/25 transition-colors disabled:opacity-50"
                >
                  Accept
                </button>
              )}
        ...
        {inactiveEntries.length > 0 && (
          <details className="group">
            <summary className="px-4 py-2 text-xs text-text-muted cursor-pointer hover:text-text-secondary list-none flex items-center gap-1">
            ...
            {inactiveEntries.map((entry) => (
              <div key={entry.id} className="px-4 py-2.5 border-b border-rule-faint opacity-50">
                <div className="flex items-center gap-3">
                  <div className="w-6 shrink-0" />
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className="h-5 w-5 rounded bg-surface-inset flex items-center justify-center shrink-0 text-xs font-bold text-text-muted">
                      {entry.club?.name?.[0]?.toUpperCase() ?? "?"}
                    </div>
                    <span className="truncate text-xs text-text-muted">{entry.club?.name ?? "Unknown"}</span>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className="text-xs text-text-muted tabular-nums">
                      {entry.fee_amount != null ? formatCurrency(entry.fee_amount) : "TBD"}
