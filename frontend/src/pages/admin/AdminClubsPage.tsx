...
function WorldTeamPicker({
  ...
  return (
    <div ref={ref} className="relative">
      <label className="mb-1 block text-xs text-text-muted">Search world team to prefill</label>
      <input
        value={q}
      ...
      {open && data && data.items.length > 0 && (
        <div className="absolute top-full left-0 right-0 z-50 mt-1 rounded-xl bg-surface ring-1 ring-border shadow-xl overflow-hidden">
          {data.items.map((team) => (
            <button
              key={team.id}
              type="button"
              onClick={() => { onSelect(team); setQ(""); setOpen(false); }}
              className="flex w-full items-center gap-3 px-3 py-2 text-left text-sm hover:bg-surface-inset transition-colors"
            >
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-surface-inset overflow-hidden">
                {team.crest_url
  ...
  return (
    ...
    >
      ...
      {!prefilled ? (
        ...
        <div className="flex items-center gap-3 rounded-lg bg-surface-inset px-3 py-2 ring-1 ring-border">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-surface overflow-hidden">
            {prefilled.crest_url
              ? <img src={prefilled.crest_url} alt={prefilled.name} className="h-full w-full object-contain p-0.5" />
              : <span className="text-xs font-bold text-text-muted">{prefilled.name[0]}</span>}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-text truncate">{prefilled.name}</p>
            <p className="text-xs text-text-muted">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div>
          <label className="mb-1 block text-xs text-text-muted">User ID (UUID)</label>
          <input
            value={userId}
        ...
        {!prefilled && (
          <div>
            <label className="mb-1 block text-xs text-text-muted">Club name</label>
            <input
              value={name}
        ...
        {!prefilled && (
          <>
            ...
            <div>
              <label className="mb-1 block text-xs text-text-muted">League</label>
              <input
                value={league}
      ...
      <div className="flex items-center gap-3">
        ...
        {mutation.isError && (
          <p className="text-xs text-danger-text">{getApiError(mutation.error, "Create failed.")}</p>
        )}
        {mutation.isSuccess && (
          <p className="text-xs text-success-text">Club created.</p>
        )}
      </div>
    </form>
  );
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="rounded-lg bg-surface px-3 py-2 text-sm text-text placeholder-text-muted ring-1 ring-input-border focus:outline-none focus:ring-warning-fill transition-colors w-56"
          />
      ...
      {showCreate && (
        <div className="mb-6 rounded-xl bg-surface ring-1 ring-border px-5 py-4">
          <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-text-muted">New club</p>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-surface-inset text-xs font-bold text-text-muted overflow-hidden">
                          {club.crest_url ? (
                        ...
                        <span className="font-medium text-text">{club.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <Badge variant={ROLE_VARIANT[club.role] ?? "neutral"}>{club.role}</Badge>
                    </td>
                    <td className="px-4 py-3 text-text-muted">{club.league_name ?? "—"}</td>
                    <td className="px-4 py-3 text-text-muted">{club.country ?? "—"}</td>
                    <td className="px-4 py-3 text-xs text-text-muted">{formatDate(club.created_at)}</td>
                    <td className="px-4 py-3 text-xs text-text-muted">{club.id.slice(0, 8)}…</td>
                  </tr>
                ))}
              </tbody>
            </table>
