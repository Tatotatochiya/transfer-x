...
**Outstanding work:**
- **⚠️ Run `backend/scripts/repair_listings_left_open_after_offer_accepted.py` against Railway**, after the `accept_offer` fix is committed and deployed there. Railway carries the same broken rows local did (listings left `OPEN` behind a live deal, `deal.sale_id` unset). It is not an Alembic migration, so nothing runs it automatically on deploy. Idempotent; use `--dry-run` first. Tracked in [`operations/environments-and-deployment.md`](./operations/environments-and-deployment.md) → "Pending data repairs on Railway" — delete the row there once done.
